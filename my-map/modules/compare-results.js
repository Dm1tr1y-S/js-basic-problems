...
export default testObj => {
    ...
    if (!compareArrays(actualResult, expectedResult)) {
        throw new Error(`${functionName}(${JSON.stringify(arr)}, ${callback}) doesn't equal ${JSON.stringify(expectedResult)} for Test Case ${testCaseIndex + 1}\nActual result: ${JSON.stringify(actualResult)}`);
    }

    console.log(`  ${functionName}(array, callback) equals ${JSON.stringify(expectedResult)}`);
};